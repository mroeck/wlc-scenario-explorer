...
These packages group Avoid/Shift/Improve measures presented by @Alaux2024CRRS to simplify the tool and improve usability. To modify the alternative scenarios, users can define the intensity of implementation (diffusion) assumed across the different EU Member States (MS). The MS-specific implementation rates are based on the assessment of MS capacity using a characterization of 'low', 'medium', 'high' capacity levels in 2020 as well as future years 2030, 2040, 2050, respectively. The specific market shares assumed in the model for low/medium/high depend on the specific strategies and underlying measures, as presented in [@Alaux2024CRRS]. In the tool's custom scenario definition interface, there are four setpoints for scaling the assumed market diffusion per MS:
* 0.0 (0% scaling, i.e. no implementation of the strategy, a BAU trajectory)
* 0.5 (50% scaling, i.e. half implementation acc. to established MS capacity)
* 1.0 (100% scaling, i.e. full implementation acc. to established MS capacity)
* max (‘high’ diffusion across all MS, overwriting prior low and medium MS capacity).
